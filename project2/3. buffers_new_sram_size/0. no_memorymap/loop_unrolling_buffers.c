...
#define finalfilename "akiyo_loop_unrolling_buffers_y.yuv"
...
/*Edw dinetai to filtrarisma me ton 2d pinaka highPassFilter*/
	for(i=1;i<N+1;i++){
		...
		for(j=1;j<M+1;j+=4){
			newtemparray[i][j]=(buffer2[j] * 8 + // pollaplasiazei to idio stoixeio me 8
                          buffer1[j-1] *(-1) +   // stoixeio panw aristera
                          buffer2[j-1]   *(-1) +   // stoixeio aristera
                          buffer3[j-1] *(-1) +   // stoixeio katw aristera
                          buffer1[j]   *(-1) +   // stoixeio apo panw
                          buffer3[j]   *(-1) +   // stoixeio apo katw
                          buffer1[j+1] *(-1) +   // stoixeio panw deksia
                          buffer2[j+1]   *(-1) +   // stoixeio deksia
                          buffer3[j+1] *(-1)  // stoixeio katw deksia
                          );

      newtemparray[i][j+1]=(buffer2[j+1] * 8 + // pollaplasiazei to idio stoixeio me 8
                          buffer1[j] *(-1) +   // stoixeio panw aristera
                          buffer2[j]   *(-1) +   // stoixeio aristera
                          buffer3[j] *(-1) +   // stoixeio katw aristera
                          buffer1[j+1]   *(-1) +   // stoixeio apo panw
                          buffer3[j+1]   *(-1) +   // stoixeio apo katw
                          buffer1[j+2] *(-1) +   // stoixeio panw deksia
                          buffer2[j+2]   *(-1) +   // stoixeio deksia
                          buffer3[j+2] *(-1)  // stoixeio katw deksia
                          );

      newtemparray[i][j+2]=(buffer2[j+2] * 8 + // pollaplasiazei to idio stoixeio me 8
                          buffer1[j+1] *(-1) +   // stoixeio panw aristera
                          buffer2[j+1]   *(-1) +   // stoixeio aristera
                          buffer3[j+1] *(-1) +   // stoixeio katw aristera
                          buffer1[j+2]   *(-1) +   // stoixeio apo panw
                          buffer3[j+2]   *(-1) +   // stoixeio apo katw
                          buffer1[j+3] *(-1) +   // stoixeio panw deksia
                          buffer2[j+3]   *(-1) +   // stoixeio deksia
                          buffer3[j+3] *(-1)  // stoixeio katw deksia
                          );

      newtemparray[i][j+3]=(buffer2[j+3] * 8 + // pollaplasiazei to idio stoixeio me 8
                          buffer1[j+2] *(-1) +   // stoixeio panw aristera
                          buffer2[j+2]   *(-1) +   // stoixeio aristera
                          buffer3[j+2] *(-1) +   // stoixeio katw aristera
                          buffer1[j+3]   *(-1) +   // stoixeio apo panw
                          buffer3[j+3]   *(-1) +   // stoixeio apo katw
                          buffer1[j+4] *(-1) +   // stoixeio panw deksia
                          buffer2[j+4]   *(-1) +   // stoixeio deksia
                          buffer3[j+4] *(-1)  // stoixeio katw deksia
                          );
		}
	}
